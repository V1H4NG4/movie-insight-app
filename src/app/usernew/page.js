...
import {
  ...
  ChevronLeft, ChevronRight, Loader2, AlertTriangle, RefreshCw, Clock, Search
} from 'lucide-react';
      ...
      {movies.map(m => (
        <div key={m.id} className="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden group">
          ...
          <div className="p-3">
            ...
            <button
              disabled
              title="Wishlist will be enabled after user implementation"
              className="mt-3 w-full text-xs py-2 rounded-lg bg-slate-800 text-slate-400 border border-slate-700 cursor-not-allowed"
            >
          ...
          {active === 'wishlist' && (
            <section className="space-y-4">
              <EmptyCard
                ...
                subtitle="You will be able to create a wishlist only from this month's movies."
              />
            </section>
          )}
          ...
          {active === 'planner' && (
            <section className="space-y-4">
              <EmptyCard
                ...
                subtitle="Plan your watch schedule once wishlist is enabled."
              />
            </section>
          )}
